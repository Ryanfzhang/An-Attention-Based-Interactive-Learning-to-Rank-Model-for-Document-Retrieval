...
class Decoder(nn.Module):
    ...
    def forward(self, embedded_inputs, decoder_input_0, hidden_0, context, label):
        ...
        mask = self.mask.repeat(seq_len).unsqueeze(0).repeat(bs, 1).bool()
...
class attention(nn.Module):
    def __init__(self, args):
        ...
        self.w = nn.Linear(args.hidden_size, 1)
    def forward(self, batch_ranks, label):
        bs, seq_len, _ = batch_ranks.size()
        batch_doc_reprs = self.fnn(batch_ranks)
        out = self.Encoder(batch_doc_reprs, batch_doc_reprs)
        score = self.w(out).squeeze(-1)
        score = nn.functional.softmax(score, -1)
        rewards = (2**label-1.)/(torch.log2(torch.arange(label.size(1)).float()+2.)).to(device)
        ...
        R = R*(1/self.args.discount)**torch.arange(seq_len).float().to(device)
        self.rl_loss = -torch.sum( R*torch.log(score))

        std = nn.functional.softmax(label, -1)
        self.loss = -torch.sum( std * torch.log(score))
        return score
    def get_parameters(self):
        return list(self.parameters())

class MDPrank(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.fnn = nn.Sequential(
    ...
    def forward(self, batch_ranks, label):
        bs, seq_len, _ = batch_ranks.size()
        score = torch.nn.functional.softmax(self.fnn(batch_ranks).squeeze(2), dim =1)# bs, seq_len
        p, index = torch.sort(score, dim =1)
        ideal_label = torch.gather(label, 1, index)
        dcg = (2**ideal_label.float()-1)/torch.log(torch.arange(seq_len).float()+2.)
        dcg = dcg.to(device)
        rewards = (self.args.discount**(torch.arange(seq_len).unsqueeze(0).expand(bs, -1)).float().to(device))*dcg 
        R = torch.cumsum( rewards.flip((1)), dim =1).flip((1))
        R = R*(1/self.args.discount)**torch.arange(seq_len).float().to(device)
        self.loss = -torch.sum(R*torch.log(p))
        return score
...
class attentionListNet(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.fnn = nn.Sequential(
        ...
        self.Encoder = Encoder(args)
        self.w = nn.Parameter(torch.rand(args.hidden_size),requires_grad=True)
        self._init()
    ...
    def forward(self, batch_ranks, label):
        bs, seq_len, _ = batch_ranks.size()
        batch_doc_reprs = self.fnn(batch_ranks)
        outs = self.Encoder(batch_doc_reprs, batch_doc_reprs)
        scores = nn.functional.softmax(outs@self.w, dim =1)
        std = nn.functional.softmax(label, dim =-1)
        self.loss = torch.sum(-torch.sum(std * torch.log(scores),dim =1))
        return scores
    def get_parameters(self):
        return list(self.parameters())

class attentionWithHidden(nn.Module):
    ...
    def forward(self, batch_ranks, label):
        ...
        score = nn.functional.softmax(outs@self.w[:self.args.hidden_size], -1)
        pro, indice = score.max(1)
        one_hot_pointer = (runner== indice.unsqueeze(1).expand(-1, outs.size(1)))
        mask = mask*(~one_hot_pointer)
        embedding_mask = one_hot_pointer.unsqueeze(2).expand(-1, -1, self.args.hidden_size)
        ...
        R_list = []
        for i in range(seq_len):
            R_list.append(torch.sum(rewards[:,i:]*(self.args.discount ** torch.arange(seq_len-i).float().expand(bs, -1)).float().to(device), dim = -1))
        R = torch.cat(R_list, dim =-1)
        R = R*(self.args.discount**torch.arange(seq_len).float().expand(bs, -1)).to(device)

        assert torch.sum(torch.isnan(R))==0, print(rewards, r, R)
        '''
...
class zeroattentionrl(nn.Module):
...
class attRNN(object):
    def __init__(self, args, id= None, ranking_function = None, opt="Adam"):
        self.ranking_function = ranking_function(args)
        self.args = args
        self.opt = opt
        self.lr = args.lr
    ...
    def train(self, batch_ranks, batch_std , train=True):
        ...
        self.optimizer.step()
        return self.ranking_function.loss.data
